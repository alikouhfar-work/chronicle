...
import { addEpisodeLog } from '@/features/episode/actions/addEpisodeLog';

export const ShowDetailsEpisodeCard: FC<ShowDetailsEpisodeCardProps> = ({ showId, episode }) => {
  const [isEditingEpisodeLog, setIsEditingEpisodeLog] = useState(false);
    ...
    <>
      {isEditingEpisodeLog && (
        <LibraryDetailsLogModal
          ...
          title={`Review: ${episode.name}`}
          hiddenFields={{ episodeId: episode.id }}
          initialRating={episode.tracking?.rating ?? 0}
          initialNotes={episode.tracking?.notes ?? ''}
          action={addEpisodeLog}
          onClose={() => setIsEditingEpisodeLog(false)}
        />
      )}
      <li
      ...
      >
        ...
        {isWatched && !isUpcomingEpisode && (
          <button
            onClick={() => setIsEditingEpisodeLog(true)}
            className="size-8 flex shrink-0 cursor-pointer items-center justify-center gap-1.5 self-center rounded-lg bg-white/6 text-xs text-zinc-300 hover:bg-white/12"
          >
            <IconEdit size={12} />
          </button>
        )}

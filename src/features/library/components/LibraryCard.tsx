...
export type LibraryMediaCardProps<T> = {
  media: T;
};

export const LibraryMediaCard = <T extends TrackedMedia>({ media }: LibraryMediaCardProps<T>) => {
  const statusConfig = showStatusConfig[media.tracking?.status];
  ...
  return (
    <li className="group border-zinc-850/80 relative aspect-2/3 w-full cursor-pointer overflow-hidden rounded-2xl border shadow-lg transition-all duration-300 select-none hover:-translate-y-1 hover:border-zinc-700/80 hover:shadow-2xl hover:shadow-black/40">
      <Link href={`/library/tv/${media.tmdbId}`}>
        <div
          className={`absolute inset-0 bg-gradient-to-br ${placeholderGradient} flex flex-col justify-between p-4 transition-transform duration-500 group-hover:scale-105`}
        >
          {media.posterPath && (
            <Image
              fill
          ...
          <div className="pointer-events-none absolute inset-0 bg-white/[0.01] bg-[radial-gradient(#ffffff03_1px,transparent_1px)] [background-size:16px_16px]" />
        </div>

        <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/60 to-zinc-950/10 transition-all duration-300 group-hover:via-zinc-950/65" />
        ...
        <div className="absolute inset-0 z-10 flex flex-col justify-between p-4">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-start justify-between gap-2">
              ...
              {media.tracking?.status && (
                <span
                  className={`rounded border px-2 py-1.5 font-mono text-[7px] leading-none font-black tracking-widest uppercase shadow-sm ${statusConfig.colors}`}
                >
                  {statusConfig.title}
                </span>
              )}
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              {media.tagline && (
                <p className="line-clamp-1 text-[9.5px] font-light text-zinc-300 italic opacity-90 drop-shadow-sm">
                  {media.tagline}
                </p>
              )}
            </div>
